...
Copy of pyscf.md.integrators with some modifications.
...
from pymbxas.tools.geometry import get_angular_momentum, get_center_of_mass, get_inertia_tensor
#%%
...
class NVTBerendson(_Integrator):
    ...
    # counteract rotations of the molecule
    def _zero_rotation(self):  
    
        # 1. Center of Mass (COM)
        com = get_center_of_mass(self.mol)
    
        # 2. Moments of Inertia and Angular Momentum
        Ip, basis = get_inertia_tensor(self.mol, vectors=True)
        Lp = get_angular_momentum(self.mol, self.veloc)
    
        # 3. Rotation Velocity (in principal basis)
        Lp = np.dot(basis, Lp)
        omega = np.dot(np.linalg.inv(basis), np.select([Ip > 0], [Lp / Ip]))
    
        # 4. Remove Rotation (updated)
        coordinates = self.mol.atom_coords()
        coordinates -= com  # Translate to COM
        self.veloc -= np.cross(omega, coordinates)  # Counteract rotation

        return
    
    # counteract drift in COM of the molecule
    def _zero_translation(self):
        self.veloc -= self.veloc.mean(axis=0)
        return
