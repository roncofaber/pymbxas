...
There is one end-to-end check, `tests/test_h2o_kedge.py`. It runs the full H2O oxygen K-edge (LDA/def2-SVPD, roughly 15 s) and asserts the method invariants listed in `dev/method.md`: SCF convergence, core-hole retention, `det(A)` sanity, the XCH alignment identity, and a non-empty spectrum.

```bash
conda run -n pymbxas pytest tests/ -q
```

**Run it before every commit that touches `mbxas/`, `calculators/`, `build/`, or `utils/orbitals.py`.** It is the only thing standing between a refactor and a silently wrong spectrum. There is no CI; nothing runs it for you.

Scope is deliberately one calculation. Do not grow this into a broad unit-test suite. If you need to check a new physics invariant, add an assertion to the existing test rather than a new file.
...
- **`absorption` is an amplitude, shape `(3, n_transitions)`, not an intensity.** Square it before broadening. Isotropic intensity is the *mean* over the three Cartesian components, not the sum.
...
- **`make_pyscf_calculator` takes `is_gpu`, not `gpu`, and has a `**kwargs` catch-all.** Calling it with `gpu=True` silently disabled the GPU for the entire package until this was caught. This is the same failure mode as the `ase_to_mole` kwarg drop above. When adding a parameter to either function, check every call site by name rather than assuming it arrives.
